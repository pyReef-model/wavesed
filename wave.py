...
def runWaveSed(bfile=None, rfac=2, H0=None, dir=None, perc=None, wbase=None,
          slvl=0., d50=0.0001,tsteps=1000,dsteps=1000,
          size=None, i1=0, i2=-1, j1=0, j2=-1):
    """
    ...
    for s in range(nbClims):
        ...
        waveparams.cmptwaves(source, h0=H0[s], sigma=1.)
        ...
        waveparams.cmptsed(sigma=1.,tsteps=tsteps,dsteps=dsteps)
        print '- Finished sediment transport computation for scenario ',s

        if size is not None:
            waveparams.plotData(data='erodep', figsize=size, vmin=-2., vmax=2.,
...
class wave:
    ...
    def __init__(self, filename=None, wavebase=10., resfac=1, dia=0.0001, Ce=1., Cd=30.):
        """
        ...
        self.ds = dia*np.power(self.grav*(self.rhos/self.rhow-1)/(self.nu*self.nu),1./3.)
        ...
        if not os.path.isfile(filename):
            raise RuntimeError('The given file cannot be found or the path is incomplete!')

        data = pd.read_csv(filename, sep=r'\s+', engine='c',
        ...
        if self.resfac <= 1:
            ...
            interfunction = interpolate.RectBivariateSpline(self.regX,self.regY,self.regZ)
    ...
    def cmptwaves(self, src=None, h0=0., sigma=1., shadow=0, shoalC=0.99):
        ...
        self.waveC,self.waveL,self.travel,self.waveH = ocean.wave.airymodel(self.sdx,
        ...
        travel[travel<0.] = self.travel.max()+10.
        ...
        n = 0.5*(1.+2.*k*self.depth/np.sinh(2.*k*self.depth))
        self.waveP = self.rhow*self.grav*self.waveC*n*self.waveH*self.waveH/8.
        ...
        tmp2 = np.sinh(2*np.pi*self.depth[self.sear,self.seac]/self.waveL[self.sear,self.seac])
        self.waveU[self.sear,self.seac] = tmp1 /(self.waveT[self.sear,self.seac]*tmp2)
        ...
        tmp3 = np.log(self.dia/(15.*self.depth[self.sear,self.seac]))
        fric[self.sear,self.seac] = 8./25.*np.power(1+tmp3,-2)

        # Shear stress (N/m2)
        self.waveS = 0.5*self.rhow*fric*self.waveU*self.waveU

        # Set land values to 0.
        self.waveL[self.landr,self.landc] = 0.
        self.waveD[self.landr,self.landc] = 0.
        self.waveT[self.landr,self.landc] = 0.
        self.waveP[self.landr,self.landc] = 0.
        self.waveU[self.landr,self.landc] = 0.
        self.waveS[self.landr,self.landc] = 0.

        return

    def cmptsed(self, sigma=1., tsteps=500, dsteps=1000):
        """
        ...
        self.waveS[self.waveS<1.e-4] = 0.
        r,c=np.where(self.waveS>0.)
        Hent = np.zeros(self.waveS.shape)
        Hent[r,c] = -self.Ce*np.log(np.sqrt(np.power(self.tau_cr/self.waveS[r,c],2)))
        Hent[Hent<0.] = 0.
        ...
        if sigma>0:
            ...
            self.Hent = Hent
        self.Hent[self.landr,self.landc] = 0.
        ...
        # Compute sediment transport
        dz,dist = ocean.wave.transport(tsteps,self.depth,self.Hent,tX,tY)
        ...
        ndz = ocean.wave.diffusion(elev, dz, Cdiff, maxth, CFL, dsteps)
        ...
        r,c = np.where(np.logical_and(self.dz>0,self.depth<-2.))
        self.dz[r,c] = 0.
    ...
    def outputCSV(self, filename=None, seddata=0):
        ...
        if seddata==0:
            ...
            df = pd.DataFrame({'X':tx.flatten(),'Y':ty.flatten(),'Z':tz.T.flatten(),
                               'wH':self.wH.T.flatten(),'wS':self.wS.T.flatten(),
                               'erodep':self.erodep.T.flatten()
                              })
            df.to_csv(filename,columns=['X','Y','Z','wH','wS','erodep'], sep=',', index=False)

        return

    def plotData(self, data='bathy', figsize=(25,10), tstep=10, vmin=0, vmax=0,
                 fontsize=10, stream=0, imin=0, imax=-1, jmin=0, jmax=-1,
                 save=None):
        """
        ...
        if data=='bathy':
            ...
            im1 = ax1.imshow(np.flipud(tmpZ[imin:imax,jmin:jmax].T-self.sealvl),interpolation='nearest',
                             cmap=cmo.cm.delta,vmin=vmin, vmax=vmax)

            ax1.contour(np.flipud(tmpZ[imin:imax,jmin:jmax].T-self.sealvl), 0, colors='k',  linewidths=2)
            ...
            im1 = ax1.imshow(np.flipud(self.waveL[imin:imax,jmin:jmax].T),interpolation='nearest',
                             cmap=cmo.cm.dense,vmin=vmin, vmax=vmax)
            ...
            im1 = ax1.imshow(np.flipud(self.travel[imin:imax,jmin:jmax].T),interpolation='nearest',
                             cmap=cmo.cm.amp,vmin=vmin, vmax=vmax)

            levels = np.arange(0, self.travel[imin:imax,jmin:jmax].max(), tstep)
            ax1.contour(np.flipud(self.travel[imin:imax,jmin:jmax].T), levels, colors='k',  linewidths=0.5)
            ...
            im1 = ax1.imshow(np.flipud(self.waveC[imin:imax,jmin:jmax].T),interpolation='nearest',
                             cmap=cmo.cm.tempo,vmin=vmin, vmax=vmax)
            ...
            if stream>0:
                ...
                wDirX[self.landr,self.landc] = 0.
                wDirY[self.landr,self.landc] = 0.
                U = wDirX[imin:imax,jmin:jmax]
                V = -wDirY[imin:imax,jmin:jmax]

                sX,sY = np.meshgrid(np.arange(U.shape[0]),np.arange(U.shape[1]))

                ax1.streamplot(sX, sY, np.flipud(U.T),
                              np.flipud(V.T), linewidth=1,
                              density=stream, color="w" )
            ...
            im1 = ax1.imshow(np.flipud(self.waveH[imin:imax,jmin:jmax].T),interpolation='nearest',
                             cmap=cmo.cm.deep,vmin=vmin, vmax=vmax)
            ...
            im1 = ax1.imshow(np.flipud(self.waveP[imin:imax,jmin:jmax].T),interpolation='nearest',
                             cmap=cmo.cm.matter,vmin=vmin, vmax=vmax)
            ...
            im1 = ax1.imshow(np.flipud(self.waveU[imin:imax,jmin:jmax].T),interpolation='nearest',
            ...
            im1 = ax1.imshow(np.flipud(self.waveS[imin:imax,jmin:jmax].T),interpolation='nearest',
            ...
            im1 = ax1.imshow(np.flipud(self.Hent[imin:imax,jmin:jmax].T),interpolation='nearest',
                             cmap=cmo.cm.amp,vmin=vmin, vmax=vmax)
            ...
            im1 = ax1.imshow(np.flipud(self.dz[imin:imax,jmin:jmax].T),interpolation='nearest',
                             cmap=cmo.cm.balance,vmin=vmin, vmax=vmax)
            ...
            if vmin == vmax:
                ...
                vmax = -vmin
            ax1.set_title('Erosion/deposition (m)', fontsize=fontsize)
            im1 = ax1.imshow(np.flipud(self.erodep[imin:imax,jmin:jmax].T),interpolation='nearest',
                             cmap=cmo.cm.balance,vmin=vmin, vmax=vmax)
            ...
            if vmin == vmax:
                ...
                vmax = self.wH.max()
            ax1.set_title('Wave height (m)', fontsize=fontsize)
            im1 = ax1.imshow(np.flipud(self.wH[imin:imax,jmin:jmax].T),interpolation='nearest',
                             cmap=cmo.cm.deep,vmin=vmin, vmax=vmax)
            ...
            if vmin == vmax:
                ...
                vmax = self.wS.max()
            ax1.set_title('Wave shear stress (N/m2)', fontsize=fontsize)
            im1 = ax1.imshow(np.flipud(self.wS[imin:imax,jmin:jmax].T),interpolation='nearest',
                             cmap=cmo.cm.balance,vmin=vmin, vmax=vmax)
